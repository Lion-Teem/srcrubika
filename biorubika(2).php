...
$bioooooo= ("امروز $day_name 😄 $year_number/$month_number/$day_number 🕘 $time");



 $key = extractKey($auth);
   $ddatee= encrypt('{"bio":"'.$bioooooo.'","updated_parameters":["bio"]}',$key);
post('{
  "api_version": "4",
  "auth": "'.$auth.'",
  "client": {
...
function decrypt($encodedEncryptedData, $key)
{
    $iv  = str_repeat("\x00", 16);
    return openssl_decrypt(base64_decode($encodedEncryptedData), "AES-256-CBC", $key, OPENSSL_RAW_DATA, $iv);
}
function encrypt($encodedEncryptedData, $key)
{
    $iv  = str_repeat("\x00", 16);
    return base64_encode(openssl_encrypt($encodedEncryptedData, "AES-256-CBC", $key, OPENSSL_RAW_DATA, $iv));
}
function extractKey(string $auth)
{
    $b = "";
    $b .= substr($auth, 16, 8);
    $b .= substr($auth, 0, 8);
